...
# Optional helper for HUD aggregation
func get_slot_cooldown_percent(unique_path: String) -> float:
	var folder := get_node_or_null(unique_path)
	if folder == null: return 0.0
	var arr := _collect_abilities(folder)
	if arr.is_empty(): return 0.0
	var max_rem := 0.0
	var max_cd := 0.000001
	for a: Ability in arr:
		max_rem = max(max_rem, a.get_cooldown_left())
		max_cd  = max(max_cd,  a.get_cooldown())
	return clamp(1.0 - (max_rem / max_cd), 0.0, 1.0)

# Simple dash helper used by DashAbility
func apply_dash(vec: Vector2, duration: float) -> void:
	velocity = vec
	var t := get_tree().create_timer(duration)
	t.timeout.connect(func(): velocity = Vector2.ZERO)
